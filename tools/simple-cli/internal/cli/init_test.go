...
func TestCreateMonorepoStructure_NestedPath(t *testing.T) {
	...
	if !scaffold.PathExists(fsx.OSFileSystem{}, nestedPath) {
		t.Error("nested path was not created")
	}

	if !scaffold.PathExists(fsx.OSFileSystem{}, filepath.Join(nestedPath, "README.md")) {
...
func TestPathExists(t *testing.T) {
	...
	// Existing path
	if !scaffold.PathExists(fsx.OSFileSystem{}, tmpDir) {
		t.Error("pathExists should return true for existing path")
	}

	// Non-existing path
	if scaffold.PathExists(fsx.OSFileSystem{}, filepath.Join(tmpDir, "nonexistent")) {
